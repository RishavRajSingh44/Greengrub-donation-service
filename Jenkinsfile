pipeline {
    ...
    environment {
        SERVICE_NAME          = 'greengrub-donation-service'
                ...
                withCredentials([usernamePassword(credentialsId: 'Git_Cred', usernameVariable: 'GH_USER', passwordVariable: 'GH_TOKEN')]) {
                    ...
                    withSonarQubeEnv("${SONAR_SERVER_NAME}") {
                        sh """
                            ...
                            -Dsonar.projectKey=greengrub-donation-service \
                            -Dsonar.projectName="GreenGrub - Donation Service" \
        ...
        stage('Deploy to Kubernetes') {
            steps {
                withKubeConfig([credentialsId: "${K8S_CREDENTIALS_ID}", serverUrl: "${K8S_API_SERVER}"]) {
                    ...
                    sh "kubectl rollout status deployment/donation-service -n services --timeout=780s"
                }
            }
        }
    }
    ...
    post {
        ...
        success {
            echo 'Donation Service pipeline completed successfully.'
        }
        failure {
            echo 'Donation Service pipeline failed.'
        }
    }
}
